...
# ==========================================
# Definisi Nilai Baku Pengaturan Awal Pabrik (Factory Defaults)
DEFAULTS = {
    "tf_pilihan": "4 Jam (4h)",
...
with col_action1:
    btn_simpan = st.button("💾 Simpan Setelan", help="Mengunci parameter yang Anda ketik agar tidak hilang saat halaman disegarkan.")
with col_action2:
    st.button("🔄 Reset Standar", on_click=reset_to_defaults, help="Menghapus modifikasi dan mengembalikan ke rumus presisi awal.")
...
session_filter_active = st.sidebar.checkbox("Aktifkan Filter Jam Trading", value=st.session_state.session_filter_active, help="Membatasi pembukaan posisi baru hanya pada jam bursa dengan likuiditas dan volatilitas tinggi.")
...
with col_tp1:
    tp1_ratio = st.sidebar.number_input("Rasio TP 1:", min_value=0.3, max_value=2.0, value=float(st.session_state.tp1_ratio), step=0.1)
with col_tp2:
    tp2_ratio = st.sidebar.number_input("Rasio TP 2:", min_value=1.0, max_value=5.0, value=float(st.session_state.tp2_ratio), step=0.1)
...
# Pembaruan Parameter ke Session State saat Tombol Simpan Manual Ditekan
if btn_simpan:
...
try:
    ...
    # Eksekusi Formula Matematika Teknis Indikator
    df['hma'] = ta.hma(df[src_aktif], length=int(st.session_state.length_hma))
    df['ema'] = ta.ema(df['close'], length=int(st.session_state.length_ema))
    df['rsi'] = ta.rsi(df['close'], length=int(st.session_state.length_rsi))
    df['atr'] = ta.atr(df['high'], df['low'], df['close'], length=int(st.session_state.length_atr))
    df['vol_ma'] = ta.sma(df['volume'], length=int(st.session_state.length_vol_ma))
    
    df['highest_high'] = df['high'].rolling(window=22).max()
    df['lowest_low'] = df['low'].rolling(window=22).min()
    df['chandelier_long'] = df['highest_high'] - (df['atr'] * float(st.session_state.chandelier_mult))
    df['chandelier_short'] = df['lowest_low'] + (df['atr'] * float(st.session_state.chandelier_mult))
    ...
    # Blok Pengecekan Validitas Kondisi Sinyal Berjalan
    for i in df.index:
        if i < max(int(st.session_state.length_hma), int(st.session_state.length_ema), int(st.session_state.length_atr), int(st.session_state.length_vol_ma), int(st.session_state.length_rsi), 22): 
            continue
            
        # Saringan Jam Transaksi Berdasarkan Zona Waktu Data
        is_allowed_time = True
        if st.session_state.session_filter_active:
            current_hour = df.at[i, 'date'].hour
            start_h = int(st.session_state.start_hour)
            end_h = int(st.session_state.end_hour)
            if start_h <= end_h:
                is_allowed_time = (start_h <= current_hour <= end_h)
            else:
                is_allowed_time = (current_hour >= start_h or current_hour <= end_h)
        ...
        # PERBAIKAN BUG UTAMA: Mengganti sintaksis salah murni '&&' menjadi keyword resmi Python 'and'
        if df.at[i, 'is_green'] and is_pullback_long and rsi_safe_long and volume_valid and is_allowed_time and last_signal != 1:
            df.at[i, 'buy_signal'] = True
            last_signal = 1
        elif df.at[i, 'is_red'] and is_pullback_short and rsi_safe_short and volume_valid and is_allowed_time and last_signal != -1:
    ...
    # --- BANNER KOTAK MONITOR STATUS REAL-TIME DI ATAS ---
    last_row = df.iloc[-1]
    if last_row['display_buy']:
        st.success("### 🟢 SINYAL AKTIF DETEKSI: LONG (BUY) SEKARANG! 🚀")
        st.info(f"**Panduan Entry Pasar:** Batas Masuk: ${last_row['close']:,.2f} | Estimasi Awal SL: ${last_row['close'] - (last_row['atr'] * float(st.session_state.atr_multiplier)):,.2f}")
    elif last_row['display_sell']:
        st.error("### 🔴 SINYAL AKTIF DETEKSI: SHORT (SELL) SEKARANG! 📉")
        st.info(f"**Panduan Entry Pasar:** Batas Masuk: ${last_row['close']:,.2f} | Estimasi Awal SL: ${last_row['close'] + (last_row['atr'] * float(st.session_state.atr_multiplier)):,.2f}")
    else:
        if last_row['is_green']: 
            st.info("### ⚪ STATUS MONITOR PASAR: WAIT / HOLD LONG POSITION (Tren Dominan Naik) 📈")
        else: 
            st.warning("### ⚪ STATUS MONITOR PASAR: WAIT / HOLD SHORT POSITION (Tren Dominan Turun) 📉")

    st.markdown("---")

    # ==========================================
    # ⚙️ SIMULATOR BACKTEST UTAMA + REINVESTMENT
    # ==========================================
    trades_list = []  
    active_trade = None
    current_equity = float(st.session_state.modal_awal)
    ...
    for i in df.index:
        if active_trade is not None and active_trade['Status'] == "Berjalan (Running)":
            if active_trade['Posisi'] == "🟢 LONG (Buy)":
                ...
                if df.at[i, 'low'] <= current_sl:
                    ...
                    profit_net = (profit_raw * int(st.session_state.leverage)) - (float(st.session_state.trading_fee_pct) * 2)
                    ...
                    active_trade['Status'] = "💥 Terkena Stop Loss/Trailing" if not active_trade['TP1_Hit'] else "🎯 TP1 + 💥 Trailing Sisa"
        ...
        # Alokasi Trigger Eksekusi Posisi Baru Berbasis Efek Gulung Modal (Compounding)
        if df.at[i, 'display_buy']:
            ...
            sl_price = df.at[i, 'close'] - (df.at[i, 'atr'] * float(st.session_state.atr_multiplier))
            jarak_sl = abs(df.at[i, 'close'] - sl_price)
            tp1_price = df.at[i, 'close'] + (jarak_sl * float(st.session_state.tp1_ratio))
            tp2_price = df.at[i, 'close'] + (jarak_sl * float(st.session_state.tp2_ratio))
            
            jarak_sl_pct = jarak_sl / df.at[i, 'close']
            usd_risk = current_equity * (float(st.session_state.risiko_per_trade_pct) / 100)
            margin_final = min((usd_risk / (jarak_sl_pct * int(st.session_state.leverage))), current_equity * 0.95)
            ...
            active_trade = {
                ...
                'Harga Entry (\$)': round(df.at[i, 'close'], 2), 'Harga SL (\$)': round(sl_price, 2),
                'Harga TP1 (\$)': round(tp1_price, 2), 'Harga TP2 (\$)': round(tp2_price, 2),
                'Margin Kunci (\$)': round(margin_final, 2), 'Status': "Berjalan (Running)", 'TP1_Hit': False, 'Laba_TP1_USD':
