...
# DAFTAR KOIN POPULER YANG DISATUKAN (BTC, ETH, USDT, SOL, DOGE)
...
def market_monitor_fragment():
    ...
    # 1. Menampilkan Dompet Terpadu di Layar Utama HP
    ...
    # 2. Perulangan Otomatis Berjalan Beriringan Memproses Semua Koin Sekaligus
    for pair in LIST_PAIRS:
        ...
        try:
            ...
            # Membaca bar terdepan [-1] agar deteksi sinyal super instan
            current_hma = df['hma'].iloc[-1]
            prev_hma = df['hma'].iloc[-2]
            current_close = df['close'].iloc[-1]
            
            is_green = current_hma >= prev_hma
            raw_buy = is_green and last_signal_pair != 1
            ...
            if mode_input == "Live Trading Real" and api_input and secret_input:
                try:
                    # Tampilkan info koin real di bawah IDR jika koin tersebut dimiliki
            ...
            if not active_trades.empty:
                for idx, row in active_trades.iterrows():
                    ...
                    # A. Proteksi Darurat: Sinyal Hilang & Garis Berbalik Merah (Auto-Cut Loss Instan)
                    if not is_green:
                        if mode_input == "Live Trading Real" and api_input and secret_input:
                        ...
                        st.toast(f"🟥 {pair} Sinyal Palsu Lenyap! Bot Langsung Cut Loss Otomatis.", icon="🚨")
                        st.rerun()
            ...
            if raw_buy:
                ...
                if nominal_belanja >= 10000:
                    ...
                    save_setting(last_sig_key, num_val=1.0) # Kunci koin ini agar tidak beli beruntun
    ...
    if not all_active.empty:
        # Tampilkan floating profit real-time per koin yang sedang jalan
